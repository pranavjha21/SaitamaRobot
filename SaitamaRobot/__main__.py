...
def start(update: Update, context: CallbackContext):
    ...
    if update.effective_chat.type == "private":
        ...
        update.effective_message.reply_text(
            "I'm moti - matab anuskha (jante hi hoge)@its_Anushka_08\n<b>Haven't slept since:</b> <code>{}</code>"
            .format(uptime),
...
def main():
    ...
    if SUPPORT_CHAT is not None and isinstance(SUPPORT_CHAT, str):
        try:
            dispatcher.bot.sendMessage(f"@{SUPPORT_CHAT}", "good night chlo mai chala sone (mai chota sa naha sa baccha ho) matab kiido !")
        except Unauthorized:
            LOGGER.warning(
